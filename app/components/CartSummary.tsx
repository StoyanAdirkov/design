    ...
    <div className={layout === 'aside'
      ...
      {layout === 'aside' ? (
        <Link to="/cart" className="flex items-center justify-center gap-2 py-3 bg-dark text-light rounded-lg font-semibold text-sm no-underline hover:bg-gray-900 hover:no-underline transition-colors duration-150" onClick={close}>
          Към поръчката <ArrowRightIcon className="size-4" />
        </Link>
        ...
        <button disabled className="inline-flex py-3 px-8 bg-brand/50 text-white border-none rounded-lg font-semibold text-sm cursor-not-allowed">Към поръчката</button>
      )}
    </div>
  );
}
